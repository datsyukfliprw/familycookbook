...
export const recipeSearchText = (recipe: Recipe) => [
...
export const searchRecipes = (recipes: Recipe[], rawQuery: string): Recipe[] => {
  const query = rawQuery.trim().toLowerCase();
  if (!query) return recipes;
  const under = query.match(/under\s+(\d+)\s*(?:min|minutes?)?/);
  const terms = query
    .replace(/under\s+\d+\s*(?:min|minutes?)?/, '')
    .replace(/\bwith\b/g, ' ')
    .split(/\s+/)
    .filter(Boolean);
  return recipes.filter(recipe => {
    if (under) {
      const limit = Number(under[1]);
